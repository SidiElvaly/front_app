...
export default function Topbar({ title }: { title: string }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-100 bg-white/80 backdrop-blur">
      {/* Safe-area padding for iOS + compact on mobile */}
      <div className="px-3 pt-[calc(env(safe-area-inset-top)+10px)] pb-3 sm:px-4 md:px-6 md:pt-3">
        <div className="flex items-center justify-between gap-3">
          {/* Left side */}
          <div className="flex min-w-0 items-center gap-3">
            {/* Brand badge: hide on very small screens to save space */}
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-3">
            ...
            {/* User: compact on mobile (initials only), full chip on sm+ */}
            <Link
